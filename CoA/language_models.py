...
class Claude:
    ...
    def __init__(self, model_name, api_key=config.ANTHROPIC_API_KEY) -> None:
        ...
        self.api_key = api_key
        if config.IS_USE_PROXY_OPENAI:
            self.model = anthropic.Anthropic(api_key=self.api_key, proxies=config.PROXY)
        else:
...
class PaLM:
    ...
    def generate(self, conv: List, max_n_tokens: int, temperature: float, top_p: float):
        ...
        for _ in range(self.API_MAX_RETRY):
            try:
                completion = palm.chat(messages=conv, temperature=temperature, top_p=top_p)
...
class OpenSourceModelAPI(LanguageModel):
    ...
    def batched_generate(
        self, full_prompts: List, max_n_tokens: int, temperature: float, top_p: float, is_get_attention: bool = False
    ):
        """
        Args:
            conv: List of dictionaries,
        ...
        for full_prompt in full_prompts:
            response = requests.get(
                "http://localhost:" + str(config.port) + "/llm_gen",
                json={
...
class CommercialAPI(LanguageModel):
    ...
    def batched_generate(
        self, conv: List, max_n_tokens: int, temperature: float, top_p: float, is_get_attention: bool = False
    ):
        """
        Args:
            conv: List of dictionaries,
    ...
    def batched_generate_by_thread(
        ...
        def thread_func(conv, max_n_tokens, temperature, top_p):
            result = self.batched_generate(conv, max_n_tokens, temperature, top_p)
            results.append(result)

        for conv in convs_list:
            thread = threading.Thread(target=thread_func, args=(conv, max_n_tokens, temperature, top_p))
