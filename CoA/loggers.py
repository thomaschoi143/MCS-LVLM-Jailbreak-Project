...
#         if (
#             task.get_now_rounds()[0] == task.get_max_rounds()[0]
#             and task.get_judge_scores()[0] >= config.THRESHOLD_JUDGE_SCORE
#         ):
...
class LocalLogger:
    ...
    def finish(self):
        ...
        if self.is_jailbroken:
            self.logger["final_response"] = self.attack_chain_result[str(len(self.attack_chain_result))][
                "target_response"
            ]
