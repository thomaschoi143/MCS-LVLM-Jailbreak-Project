...
def extract_json(s):
    ...
    json_str = s[start_pos:end_pos]
    json_str = json_str.encode("utf-8").decode("unicode_escape")  # Decode unicode escape sequences
...
def get_mr_init_chain(args, model):
    ...
    if args.language == "en":
        mr_attack_chain_system_prompt = get_mr_attack_chain_system_prompt_en(args.target, args.max_round)
    elif args.language == "cn":
        mr_attack_chain_system_prompt = get_mr_attack_chain_system_prompt_cn(args.target, args.max_round)

    init_msg_mr = get_init_msg_for_mr_init_chain(args.target, args.max_round, language=args.language)
    processed_response_mr_list = [init_msg_mr for _ in range(args.n_mr_init_chain)]

    convs_mr_list = [conv_template(model.template) for _ in range(args.n_mr_init_chain)]
    ...
    if args.is_get_mr_init_chain:
        ...
        while retry_mr_init <= args.retry_mr_init_chain:
            ...
            try:
                ...
                # remove None
                extracted_mr_init_chain_reponse = [item for item in extracted_mr_init_chain_reponse if item != None]
...
def process_mr_response(
    ...
    if methods == "JFTS":
        topk_toxic_scores_index = sorted(
            range(len(toxic_scores)),
            key=lambda i: (judge_scores[i], toxic_scores[i]["toxicity"] + toxic_scores_prompt[i]["toxicity"]),
            reverse=True,
        )[:topk]
    elif methods == "JPT":
        topk_toxic_scores_index = sorted(
            ...
            key=lambda i: (
                judge_scores[i]
                + math.ceil(toxic_scores[i]["toxicity"] * 10)
                + math.ceil(toxic_scores_prompt[i]["toxicity"] * 10)
            ),
            reverse=True,
        )[:topk]
    elif methods == "TFJS":
        topk_toxic_scores_index = sorted(
            range(len(toxic_scores)),
            key=lambda i: (toxic_scores[i]["toxicity"] + toxic_scores_prompt[i]["toxicity"], judge_scores[i]),
            reverse=True,
        )[:topk]
    elif methods == "SPT":
        # pass
        topk_toxic_scores_index = sorted(
            range(len(toxic_scores)), key=lambda i: (toxic_scores[i]["toxicity"] + sem_scores_response[i]), reverse=True
        )[:topk]
        ...
        topk_toxic_scores_index = sorted(
            range(len(toxic_scores)), key=lambda i: (sem_scores_response[i], toxic_scores[i]["toxicity"]), reverse=True
        )[:topk]
    elif methods == "JFSSTS":
        topk_toxic_scores_index = sorted(
            range(len(toxic_scores)),
            key=lambda i: (judge_scores[i], sem_scores_response[i], toxic_scores[i]["toxicity"]),
            reverse=True,
        )[:topk]
    else:
        raise NotImplementedError
...
def process_mr_init_chain(args, mr_init_chain, toxic_model, sem_judger, method="SEM", topk=1):
    ...
    for i in range(len(mr_init_chain)):
        sem_score = [sem_judger.compute_similarity(args.target, item["prompt"]) for item in mr_init_chain[i]["mr_conv"]]
        toxic_scores = [
            item["toxicity"]
            for item in toxic_model.get_batched_toxicity([item["prompt"] for item in mr_init_chain[i]["mr_conv"]])
        ]
    ...
    for i in range(len(sorted_mr_init_chain)):
        sorted_mr_init_chain[i]["sem_score"] = [item["sem_score"] for item in sorted_mr_init_chain[i]["mr_conv"]]
        sorted_mr_init_chain[i]["toxic_score"] = [item["toxicity"] for item in sorted_mr_init_chain[i]["mr_conv"]]
    ...
    if sorted_method == "range":
        ...
        sorted_mr_init_chain = sorted(
            sorted_mr_init_chain,
            key=lambda x: (max(x["sem_score"]) - min(x["sem_score"]), max(x["toxic_score"]) - min(x["toxic_score"])),
            reverse=True,
        )
        ...
        sorted_mr_init_chain = sorted(
            sorted_mr_init_chain,
            key=lambda x: (sum(x["sem_score"]) / len(x["sem_score"]), sum(x["toxic_score"]) / len(x["toxic_score"])),
            reverse=True,
        )
...
def fixed_score(rd_manager, task):
    ...
    for i in range(len(task.task_messages)):
        ...
        if rd_info[i]["sem_response"] * (1 - theta_sem) > (sem_response[i]):
            ...
            rd_info[i]["sem_response"] * (1 - theta_sem)
            < (sem_response[i])
            < rd_info[i]["sem_response"] * (1 + theta_sem)
        ):
            # random value 2-3
        ...
        if rd_info[i]["toxic_response"] * (1 - theta_toxic) > (toxic_response[i]):
            ...
            rd_info[i]["toxic_response"] * (1 - theta_toxic)
            < (toxic_response[i])
            < rd_info[i]["toxic_response"] * (1 + theta_toxic)
        ):
            # random value 2-3
