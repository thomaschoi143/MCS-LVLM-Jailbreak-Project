...
class OpenAIModerationAPI(ToxicDetector):
    ...
    def get_batched_toxicity(self, text_list):
        ...
        while retry_times > 0:
            retry_times -= 1
            try:
                response = openai.Moderation.create(model=self.model_name, input=text_list)
        ...
        for category in categories_scores:
            sorted_scores.append(sorted(category.items(), key=lambda x: x[1], reverse=True)[0])
...
class ToxiGen(ToxicDetector):
    ...
    def __init__(self, name):
        ...
        self.base_url = f"http://localhost:{config.port}/toxigen"
