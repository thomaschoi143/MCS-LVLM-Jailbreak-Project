...
def load_attack_and_target_models(args):
    ...
    attackLM = AttackLM(
        ...
        temperature=ATTACK_TEMP,  # init to 0.3
        top_p=ATTACK_TOP_P,  # init to 0.7
    )
...
class AttackLM:
    ...
    def get_attack_mr_init_chain(self, convs_list, prompt_list):
        ...
        assert len(convs_list) == len(prompt_list), (
            "Mismatch between number of conversations and prompts. Conv List:"
            + str(len(convs_list))
            + " Prompt List:"
            + str(len(prompt_list))
        )
        ...
        for attempt in range(self.max_n_attack_attempts):
            ...
            # Generate outputs
            outputs_list, _ = self.model.batched_generate_by_thread(
            ...
            for i, full_output in enumerate(outputs_list):
                ...
                mr_conv, evaluation, json_str = common.extract_json_for_mr_init_chain(full_output)

                if mr_conv is not None and evaluation is not None:
                    valid_outputs[orig_index] = {"mr_conv": mr_conv, "evaluation": evaluation}
                else:
                    new_indices_to_regenerate.append(orig_index)
    ...
    def get_attack(self, convs_list, prompts_list, pre_prompt_sem, sem_judger, target):
        ...
        for attempt in range(self.max_n_attack_attempts):
            ...
            for i, full_output in enumerate(outputs_list):
                ...
                if (
                    attack_dict is not None
                    and attack_dict["prompt"] != "[new prompt]"
                    and attack_dict["prompt"] != "[新提示]"
                ):
        ...
        valid_outputs = [output for output in valid_outputs if output is not None]
...
class TargetLM:
    ...
    def get_response(self, convs_list, is_get_attention=False):
        ...
        for attamp in range(retry_attempts):
            ...
            full_prompts_subset = [full_prompts[i] for i in indices_to_regenerate]
            # print("Full prompts subset 0:", full_prompts_subset[0])
            ...
            outputs_list = [self.get_victim_generation(prompt) for prompt in full_prompts_subset]
    ...
    def get_victim_generation(self, conv: List):

        payload = dict(max_new_tokens=self.max_n_tokens, temperature=self.temperature, top_p=self.top_p)
        try:
            endpoint = "lvlm_gen"
            payload["conv"] = conv
            files = None
...
def get_model_path_and_template(model_name):
    ...
    """
    full_model_dict = {
        "vicuna-api": {"path": "vicuna_v1.1", "template": "vicuna_v1.1"},
        "llama2-api": {"path": "llama-2", "template": "llama-2"},
        "chatglm-api": {"path": "chatglm", "template": "chatglm"},
        "chatglm2-api": {"path": "chatglm-2", "template": "chatglm-2"},
        "phi2-api": {"path": "phi2", "template": "phi2"},
        "zephyr-api": {"path": "zephyr", "template": "zephyr"},
        "baichuan-api": {"path": "baichuan2-chat", "template": "baichuan2-chat"},
        "one-shot": {"path": "one_shot", "template": "one_shot"},
        "zhipu": {"path": "zhipu", "template": "zhipu"},
        "douyin": {"path": "douyin", "template": "douyin"},
        "wenxinyiyan": {"path": "wenxinyiyan", "template": "wenxinyiyan"},
        "kuaishou": {"path": "kuaishou", "template": "kuaishou"},
        "baichuan": {"path": "baichuan", "template": "baichuan"},
        "zero-shot": {"path": "zero_shot", "template": "zero_shot"},
        "airoboros-1": {"path": "airoboros_v1", "template": "airoboros_v1"},
        "airoboros-2": {"path": "airoboros_v2", "template": "airoboros_v2"},
        "airoboros-3": {"path": "airoboros_v3", "template": "airoboros_v3"},
        "koala-1": {"path": "koala_v1", "template": "koala_v1"},
        "alpaca": {"path": "alpaca", "template": "alpaca"},
        "chatglm": {"path": "chatglm", "template": "chatglm"},
        "chatglm-2": {"path": "chatglm-2", "template": "chatglm-2"},
        "dolly-v2": {"path": "dolly_v2", "template": "dolly_v2"},
        "oasst-pythia": {"path": "oasst_pythia", "template": "oasst_pythia"},
        "oasst-llama": {"path": "oasst_llama", "template": "oasst_llama"},
        "tulu": {"path": "tulu", "template": "tulu"},
        "stablelm": {"path": "stablelm", "template": "stablelm"},
        "baize": {"path": "baize", "template": "baize"},
        "chatgpt": {"path": "chatgpt", "template": "chatgpt"},
        "bard": {"path": "bard", "template": "bard"},
        "falcon": {"path": "falcon", "template": "falcon"},
        "baichuan-chat": {"path": "baichuan_chat", "template": "baichuan_chat"},
        "baichuan2-chat": {"path": "baichuan2_chat", "template": "baichuan2_chat"},
        "falcon-chat": {"path": "falcon_chat", "template": "falcon_chat"},
        "gpt-4": {"path": "gpt-4", "template": "gpt-4"},
        "gpt-4-turbo": {"path": "gpt-4-turbo", "template": "gpt-4-turbo"},
        "gpt-3.5-turbo": {"path": "gpt-3.5-turbo", "template": "gpt-3.5-turbo"},
        "text-davinci-003": {"path": "text-davinci-003", "template": "text-davinci-003"},
        "gpt-3.5-turbo-instruct": {"path": "gpt-3.5-turbo-instruct", "template": "gpt-3.5-turbo-instruct"},
        "vicuna": {"path": "vicuna-api", "template": "vicuna_v1.1"},
        "llama2-chinese": {"path": "llama2_chinese", "template": "llama2_chinese"},
        "llama-2": {"path": "llama2-api", "template": "llama-2"},
        ...
        "mistral-api": {"path": "mistral", "template": "mistral"},
    }
