...
class LanguageModel:
    ...
    def batched_generate_by_thread(
        self, convs_list: List[List[Dict]], max_n_tokens: int, temperature: float, top_p: float
    ):
        """
...
def load_tokenizer_and_model(model_name):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True, device_map="auto", trust_remote_code=True
    )

    if "llama-2" in model_name.lower():
        tokenizer.pad_token = tokenizer.unk_token
        tokenizer.padding_side = "left"
    if "vicuna" in model_name.lower() or "mistral" in model_name.lower():
...
def main(args):
    ...
    for model_name in args.model_name:
        ...
        if "mistral" in model_name.lower():
            lms["mistral"] = load_tokenizer_and_model(model_name)
        if "llava" in model_name.lower():
            lms["llava"] = load_tokenizer_and_model(model_name)
    ...
    if lms.get("llama2", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["llama2"].batched_generate(
    ...
    if lms.get("vicuna", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["vicuna"].batched_generate(
                full_prompts_list, max_n_tokens, temperature, top_p, is_get_attention
            )
    ...
    if lms.get("chatglm", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["chatglm"].batched_generate(
                full_prompts_list, max_n_tokens, temperature, top_p, is_get_attention
            )
    ...
    if lms.get("phi2", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["phi2"].batched_generate(
                full_prompts_list, max_n_tokens, temperature, top_p, is_get_attention
            )
    ...
    if lms.get("zephyr", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["zephyr"].batched_generate(
                full_prompts_list, max_n_tokens, temperature, top_p, is_get_attention
            )
    ...
    if lms.get("baichuan", None) != None:
        ...
        async def generate(request: Request):
            ...
            output_list, token2attn = lms["baichuan"].batched_generate(
                full_prompts_list, max_n_tokens, temperature, top_p, is_get_attention
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["mistral_small_24b_instruct_2501", "llava_v1.6_mistral_7b_hf"],
        choices=[
    ...
    parser.add_argument("--log_level", type=str, default="info", help="log_level")
