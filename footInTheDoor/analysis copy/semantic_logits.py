...
class SemanticLogitAnalyzer:
    ...
    def compute_semantic_logit_bias(self, prompt: str) -> Dict[str, float]:
        """
        ...
        with torch.no_grad():
            ...
            last_token_logits = outputs.logits[0, -1, :].cpu().numpy()
        ...
        # 使用softmax获取概率
        logit_probs = torch.nn.functional.softmax(torch.tensor(last_token_logits), dim=0).numpy()
        ...
        for idx in top_indices:
            ...
            # 获取该token的embedding
            token_embedding = self.base_analyzer.model.get_input_embeddings().weight[idx].cpu().numpy()
    ...
    def analyze_semantic_logit_boundaries(
        ...
        for i, prompt in enumerate(tqdm(fitd_sequence)):
            try:
                # 使用语义方法计算logit偏差
                logit_data = self.compute_semantic_logit_bias(prompt)
                ...
                print(f"计算级别{i+1}的logit边界时出错: {e}")
                # 添加默认值
                results.append({
                    "level": i + 1,
                    "prompt": prompt,
