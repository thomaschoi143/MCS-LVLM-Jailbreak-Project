...
def get_target_benign_sentence(index: int, harmful_dataset, benign_dataset, llm, sampling_params) -> Tuple[str, str]:
    ...
    if benign_dataset is not None:
        benign_sentence = benign_dataset.iloc[index]["task"]
...
def slipperySlopeParaphrase(
    ...
    for _ in range(2):
        ...
        answer, messages = polish_content(
            prompts[idx - 1], intermediate_prompt, messages, llm, sampling_params
        )  # Added the polished jailbreak prompt to messages
        total_query += 2
...
if __name__ == "__main__":
    ...
    # steps (number of prompts)
    parser.add_argument("--level", type=int, default=3, help="The prompt level for generating prompts")
    parser.add_argument(
        "--max_attempts", type=int, default=1, help="Maximum retry attempts for each index"
    )  # 3 attempts for each index
    ...
    # print(args)
    utils_llama.PORT = args.port

    # Load datasets
    harmful_dataset = pd.read_csv(os.path.join("../data", args.dataset, "data.csv"))
    if args.dataset == "jailbreakbench_original":
        benign_dataset = pd.read_csv(f"data/benign-behaviors.csv")
    else:
        benign_dataset = None

    strongreject_judge = StrongRejectJudge(utils_llama.PORT)
    judge = HarmbenchJudge(utils_llama.PORT)
    ...
    output_csv_path = create_csv_result(model_name, args.save_path, args.start)
    ...
    result_log_file = os.path.join(args.save_path, f"results_{args.start}.log")
    ...
    if args.num_tasks == 0:
        args.end = len(harmful_dataset)
    else:
        args.end = args.start + args.num_tasks

    # Process each index in the range
    for index in range(args.start, args.end):
        task = harmful_dataset.iloc[index]["task"]

        result_logger.info(f"Task {index}. {task}")
        ...
        while attempts < max_attempts and not result_1:
            ...
            utils_llama.TOTAL_TARGET_QUERY = 0
            ...
            # Final attack chain
            # prompts: benign_sentence, prompt 1, ..., prompt steps (args.level+1 prompts in total)
            ...
            # judge_res = judge.eval(target_sentence,
            ...
            if most_harmful_index is None:
                ...
                judge_res = judge.eval(target_sentence, output_results[final_step])

            else:
                # result_2 = is_jailbreak_by_jailbreakbench(target_sentence, output_results[most_harmful_index])
                judge_res = judge.eval(target_sentence, output_results[most_harmful_index])

            result_2 = False
            is_success = judge_res if isinstance(judge_res, bool) else False
            judge_score = strongreject_judge.eval(target_sentence, output_results[most_harmful_index])
            ...
            # final_success = result_1 or result_2
            # final_jailbreak_score = max(jailbreak_score_1, jailbreak_score_2)
            # decision_index = final_step
            # if jailbreak_score_2 > jailbreak_score_1:
            #     decision_index = most_harmful_index if most_harmful_index else final_step
            ...
            results.append(
                {
                    ...
                    "rounds_num": most_harmful_index + 1,
                    "total_target_query": utils_llama.TOTAL_TARGET_QUERY,
                    "last_response": output_results[most_harmful_index],
                }
            )
            result_logger.info(
                f"\tAttempt {attempts}: {'Success' if is_success else 'Failure'}, Judge score: {judge_score}, Rounds num: {most_harmful_index+1}, Total target query: {utils_llama.TOTAL_TARGET_QUERY} - Last response: {output_results[most_harmful_index]}"
            )

        # Handle final results
        # if not result_1 and not result_2:
        #     # The last attempt was not successful
        #     result = next((i for i in range(max_attempts) if result_2_list[i]), None)

        #     if result is not None:
        #         # Attempt "result" was successful
        #         print(f"Index {result} is True in result_2_list")
        #     else:
        #         print(f"Index {index} failed after {max_attempts} attempts.")
        #         # Select the first 2 attempts' results
    ...
    average_total_target_query = results_df[results_df["is_success"]]["total_target_query"].mean()
    result_logger.info(
        f"Binary ASR: {binary_asr}; Average ASR: {average_asr}; Average Score: {average_score}; Average number of rounds in successful attempts: {average_rounds_num}; Average number of queries in successful attempts: {average_total_target_query}"
    )

    stats_path = os.path.join(args.save_path, f"stats_{args.start}.csv")
