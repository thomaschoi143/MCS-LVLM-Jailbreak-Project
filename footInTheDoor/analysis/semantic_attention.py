...
class SemanticAttentionAnalyzer:
    ...
    def extract_attention_patterns_semantic(
        ...
        try:
            ...
            print(f"提取注意力模式成功，级别: {conversation_input.get('level', 'N/A')}, "
                  f"找到{len(safety_semantic_indices)}个安全相关token和{len(harmful_semantic_indices)}个有害相关token")
            ...
            print(f"提取级别{conversation_input.get('level', 'N/A')}的注意力模式时出错: {e}")
            return {
                "tokens": [],
                "safety_semantic_indices": [],
                "harmful_semantic_indices": [],
    ...
    def analyze_fitd_attention_patterns(
        ...
        for conversation_input in tqdm(conversation_inputs):
            ...
            # 使用语义方向方法，传入完整对话历史
            attention_data = self.extract_attention_patterns_semantic(conversation_input)
