...
class SemanticNetworkAnalyzer:
    ...
    def extract_semantic_attention_network(self, conversation_input: Dict[str, Any]) -> Dict[str, Any]:
        """
        提取基于语义的注意力网络
        修改为处理对话历史记录
        ...
        try:
            ...
            print(f"提取语义注意力网络成功，级别: {conversation_input.get('level', 'N/A')}, "
                  f"安全节点数: {len(safety_nodes)}, 有害节点数: {len(harmful_nodes)}")
            ...
            print(f"提取级别{conversation_input.get('level', 'N/A')}的注意力网络时出错: {e}")
            # 返回空结果
            return {
                "tokens": [],
                "token_semantics": [],
    ...
    def analyze_semantic_network_structure(
        ...
        for conversation_input in tqdm(conversation_inputs):
            ...
            try:
                # 提取语义注意力网络，传入完整对话历史
                network_data = self.extract_semantic_attention_network(conversation_input)
                
                # 存储图
                network_by_level[level] = network_data
                ...
                # 打印一些调试信息
                print(f"级别{level}分析完成 - 安全节点数量: {len(network_data['safety_nodes'])}, " 
                      f"有害节点数量: {len(network_data['harmful_nodes'])}")
                
            except Exception as e:
                print(f"提取级别{level}的网络结构时出错: {e}")
