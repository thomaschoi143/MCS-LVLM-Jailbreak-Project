...
def get_llm_generation(system_prompt, user_prompt):
    ...
    try:
        ...
        response_text = response_text.encode("utf-8").decode("unicode_escape").strip('"')
...
def generate_batch_image_prompt(dataset_df, demonstrations):
    # model_path = "mistralai/Mistral-7B-Instruct-v0.2"

    # vllm_model = LLM(
    #     model_path, tensor_parallel_size=gpus, trust_remote_code=True, gpu_memory_utilization=0.4, enforce_eager=True
    # )
    ...
    df = pd.DataFrame(
        columns=["redteam_query", "diffusion prompt", "detailed description", "brief description", "round", "policy"]
    )
    round = 0
    while not set(question_list).issubset(set(df["redteam_query"].tolist())):
        ...
        outputs = [get_llm_generation(None, prompt) for prompt in batch_prompt]
        ...
        for i, output in enumerate(outputs):
            ...
            if (
                len(matches_square_brackets) == 1
                and len(matches_curly_brackets) == 1
                and len(matches_circle_brackets) == 1
            ):
                ...
                if len(diffusion_prompt) < 300:
                    new_row = {
                        "redteam_query": batch_question[i],
...
def main():
    ...
    # parser.add_argument("--base", type=str, default="stabilityai/stable-diffusion-xl-base-1.0")
    ...
    if os.path.exists(f"{roles_root}/{dataset}/roles.csv"):
        ...
        dataset_df = pd.read_csv(f"{dataset_root}/{dataset}/data.csv", index_col=0)
        dataset_df.rename(columns={"task": "redteam_query"}, inplace=True)
        roles_df = generate_batch_image_prompt(dataset_df, demonstrations)
