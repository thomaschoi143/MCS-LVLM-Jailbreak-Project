...
from utils import gpt_call, read_prompt_from_file, check_file, get_client, gpt_call_append, wrapper
...
class PreAttack:
    def __init__(self, config: PreAttackConfig):
        ...
        self.org_data = df["task"].tolist()
        print(f"Loaded {len(self.org_data)} tasks from {self.behavior_csv}")
        # model
...
{"target": [TEXT], 
    "details": {"delivery_type": [TEXT], "other_details": [TEXT]}
}. 
...
"""
                data = wrapper(res, system_prompt)
...
Any deviation from outputting a JSON dictionary will result in a system crash.
"""
                data = wrapper(resp, system_prompt)
...
Any deviation from outputting a JSON dictionary will result in a system crash.
"""
                data = wrapper(json_output, system_prompt)
                queries = []
    ...
    def infer(self, num, task_i_start_from, experiment_name):
        json_data = self.config.__dict__
        if num == -1:
            num = len(self.org_data)
        else:
            num = min(num, len(self.org_data))
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = list(executor.map(self.infer_single, self.org_data[task_i_start_from : task_i_start_from + num]))
        json_data["data"] = futures
        if not os.path.exists("./pre_attack_result"):
            os.makedirs("./pre_attack_result")
        file_path = (
            f'./pre_attack_result/queries_for_{self.model_name.split("/")[-1].replace(".", "-")}_{experiment_name}_{task_i_start_from}.json'
        )
